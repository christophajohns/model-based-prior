...
class ImageSimilarityLoss(SyntheticTestFunction):
    ...
    def __init__(
        ...
        self._original_target_image = self._generate_image(torch.tensor(optimizer).reshape(1, 1, -1))  # Ensure shape (1, 1, d)
    ...
    def evaluate_true(self, X: torch.Tensor) -> torch.Tensor:
        ...
        original_candidate_image = self._generate_image(X)  # (n, q, C, H, W)
    ...
    def _generate_image(self, X: torch.Tensor) -> torch.Tensor:
        ...
        X_unit_normalized = normalize(X_flat, self.bounds)
        X_normalized = unnormalize(
            X_unit_normalized,
            torch.tensor([self._brightness_bounds, self._contrast_bounds, self._saturation_bounds, self._hue_bounds]).T
        )
        ...
        return torch.stack(images).view(n, q, *self._original_image.shape)
